...
FotobarCart.prototype.resetCart = function() {
...
FotobarCart.prototype.getGrandTotal = function() {
	...
	var total = this.getCartTotal() + this.getShippingTotal()
			+ this.getTaxTotal();
...
FotobarCart.prototype.updateQuantity = function(itemSku, itemQuantity) {
...
FotobarCart.prototype.validateCart = function() {
	...
	for (image in this.items) {
		...
		this.items[image].map(function(item) {
			...
			if (product.minQty > 0 && product.minOverride
				...
				var text = product.name + " requires a minimun of "
						+ product.minQty;
				inValid.push({
					type : 'error',
					text : text
	...
	var outcome = (inValid.length > 0) ? inValid : isValid;
	return (outcome);
};