	...
	return $.Deferred(function() {
		...
		for (var i = 0; i < imageArrayLength; i++) {
			...
			newImage.onload = function() {
				...
				//fotobar.setImageCords(fotobar.images[this.id]);
...
Fotobar.prototype.setImageParams = function(current_image) {
	...
	case (2):
		...
		$(image_container).width(this.canvasSetWidth);
		$(image_container).height(this.canvasSetHeight);
		current_image.canvas_width = this.canvasSetWidth;
		current_image.canvas_height = this.canvasSetHeight;
		break;
	...
	case (3):
		...
		current_image.canvas_width = this.canvasSetHeight;
		current_image.canvas_height = this.canvasSetWidth;
		break;
...
Fotobar.prototype.addImageEffect = function(current_image) {

	switch (current_image.effect) {

	case ("blkWht"):

		// this.blackWhite(imageData);
		break;
...
Fotobar.prototype.debugObject = function(current_image) {
	...
	for (k in current_image) {
		...
		switch (true) {
		/*
		 * case (k == 'image'): continue; break;
		 */
		case (k == 'blob'):

			current_image['blob'] = JSON.stringify(current_image['blob']);
			$("#obj").append("<br>" + 'blob' + ":" + current_image.blob);
			break;
		}
